...
public class SocketWrapperTest
{
    ...
    public void testSSLConnection_keystoreWithAlias() throws Exception
    {
        ...
        verifyConnection(2114, true, server_masterAlias, server_masterAlias, securityInfo);
    }
    
    /**
     * Verify that when specifying a wrong server alias, the server cannot start.
     * This confirms that the alias selection works
     */
    @Test
    public void testSSLConnection_keystoreWithAlias_wrong_server_alias() throws Exception
    {
        ...
        try
        {
            verifyConnection(2114, true, "alias_that_does_not_exist", client_slaveAlias, securityInfo);
            assertTrue("The server should not have started: we used a non existing alias in the keystore", false);
        }
        catch (Exception e)
        {
            assertTrue("This exception is expected as we're trying to use a non existant server alias in the keystore", true);
        }
    }
    ...
    private void verifyConnection(int port, boolean useSSL,
            String serverKeystoreAlias,String clientKeystoreAlias, AuthenticationInfo securityInfo)
            throws Exception
    {
        server = new EchoServer("127.0.0.1", port, useSSL,serverKeystoreAlias, securityInfo);
        server.start();
    ...
    {
        // Start a server.
        server = new EchoServer("127.0.0.1", port, true, null, null);
        server.start();
